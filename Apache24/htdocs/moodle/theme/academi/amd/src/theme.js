...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // *** 角色判斷邏輯 ***
    // 檢查使用者是否為 Site Admin (Admin可以看見 Site administration 連結或管理工具)
    // 如果此元素存在，則假設為高權限使用者 (Admin/Manager)
    const isAdmin = document.querySelector('.usertool_siteadministration');
    
    // 只有非 Admin 使用者 (例如學生) 才會執行以下自動化操作
    if (!isAdmin) {
        
        // 檢查是否在儀表板頁面 ('path-my')
        if (document.body.classList.contains('path-my')) {
            
            var editToggleContainer = document.querySelector('[data-region="edit-toggle"]');
            var editButton = document.querySelector('[data-action="editmode"]');

            if (editButton && editToggleContainer) {
                
                // 1. 隱藏按鈕 (只對學生隱藏)
                // 由於我們移除 Global CSS，現在改用 JS 來隱藏按鈕
                editToggleContainer.style.display = 'none';

                // 2. 自動開啟編輯模式 (如果尚未開啟)
                if (editButton.getAttribute('aria-pressed') !== 'true') {
                    
                    // 延遲執行，確保所有元素載入完畢後再模擬點擊
                    setTimeout(function() {
                        editButton.click(); 
                    }, 500); 
                }
